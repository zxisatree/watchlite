...
export default function ChannelPage({
  ...
  useEffect(() => {
    if (gapiIsInitialised) {
      ...
      console.log(`requesting for channel: ${parsedChannelHandle}`)
      gapi.client.youtube.channels
        ...
        .then(response => {
          ...
          if (!items || items.length > 1) {
            console.error(
              `unexpected number of items (not 1): ${items?.length}`,
            )
            return
          }
          setChannel(items[0])
  ...
  }, [gapiIsInitialised, incGapiRequestCount, parsedChannelHandle])
